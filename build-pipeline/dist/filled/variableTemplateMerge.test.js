...
import { mergeFilledIconIntoVariableTemplate } from "./variableTemplateMerge.js";
import { parseSvgXml, elementChildren, localTag, svgDocumentElement, } from "../svg/xml.js";
const packageRoot = join(dirname(fileURLToPath(import.meta.url)), "..", "..");
const variableTemplatePath = join(packageRoot, "resources", "square_variable_template.svg");
function slotContentGroupById(svgXml, slotId) {
    const doc = parseSvgXml(svgXml);
    const root = svgDocumentElement(doc);
    const walk = (el) => {
        if (localTag(el) === "g" && el.getAttribute("id") === slotId)
            return el;
