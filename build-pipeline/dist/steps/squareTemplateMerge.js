...
import { readViewBox } from "./sizeVariants.js";
import { WEIGHT_SIZE_SUFFIX, SF_SYMBOL_WEIGHTS } from "./weightReplicas.js";
import { parseWeightStem } from "./strokeWidths.js";
import { elementChildren, elementToBytes, localTag, parseSvgFile, SVG_NS, svgDocumentElement, } from "../svg/xml.js";
const WIREFRAME_CLASS = "SFSymbolsPreviewWireframe";
function packageRootFromModule() {
    const here = dirname(fileURLToPath(import.meta.url));
    return join(here, "..", "..");
}
export function defaultSquareTemplatePath() {
    const candidate = join(packageRootFromModule(), "resources", "square_template.svg");
    return existsSync(candidate) ? candidate : null;
}
function expectedSlotIds() {
    const s = new Set();
    for (const w of SF_SYMBOL_WEIGHTS) {
        for (const [, suf] of WEIGHT_SIZE_SUFFIX) {
            s.add(`${w}-${suf}`);
        }
    }
    return s;
...
function findSymbolsGroup(root) {
    ...
    const g = walk(root);
    if (!g)
        throw new Error('No <g id="Symbols"> found in template.');
    return g;
}
...
function fillSlot(slot, iconPath, templateDefs, templateDoc) {
    let wire = null;
...
export function runSquareTemplateMerge(templatePath, iconsDir, outputPath, options = {}) {
    const missing = options.missing ?? "skip";
    const tp = String(templatePath);
    const idir = String(iconsDir);
    const out = String(outputPath);
    if (!existsSync(tp))
        throw new Error(`Template not found: ${tp}`);
    if (!existsSync(idir))
        throw new Error(`Icons directory not found: ${idir}`);
    ...
    for (const slot of [...elementChildren(symbols)]) {
        ...
        if (!sid || !expected.has(sid))
            continue;
        if (!parseWeightStem(sid))
            continue;
        ...
        if (!existsSync(iconFile)) {
            ...
            if (missing === "warn") {
                console.warn(`Square template merge: missing icon for slot ${sid} (${iconFile}), leaving wireframe.`);
            }
            skipped++;
            continue;
        }
