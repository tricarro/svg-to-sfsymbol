...
function subpathToFilledPolygon(sp) {
    if (sp.coords.length < 2)
        return null;
    const pts = sp.coords.map(([x, y]) => [x, y]);
    const first = pts[0];
    const last = pts[pts.length - 1];
    const explicitClose = sp.closed ||
...
function fillSubpathsToGeometry(subpaths) {
    const geoms = [];
...
function circleToFillPolygon(el, segments = 48) {
...
function ellipseToFillPolygon(el, segments = 48) {
    const cx = parseFloat(el.getAttribute("cx") || "0");
    const cy = parseFloat(el.getAttribute("cy") || "0");
 ...
 */
export function unionJtsGeometries(geoms) {
...
export function jtsGeometryToSvgPathD(geom) {
    return geometryToPathD(geom);
}
export function strokeElementToOutlineGeometry(el, flatness) {
...
export function fillElementToGeometry(el, flatness) {
    if (!visibleFill(el))
        return null;
    const tag = localTag(el);
    if (tag === "path") {
...
function outlineGeometryFromSubpaths(subpaths, el, flatness) {
...
export function runStrokeToOutline(outputDir, flatness = 1) {
