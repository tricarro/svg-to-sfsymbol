...
import { classifySvgRouting, classifySvgStrokedOrFilled, elementHasVisibleStroke, } from "./svgRouting.js";
import { parseSvgString } from "../steps/sizeVariants.js";
describe("elementHasVisibleStroke", () => {
    it("false when stroke missing", () => {
        const el = parseSvgString(`<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0h10v10H0z" fill="black"/></svg>`);
        const path = el.getElementsByTagName("path")[0];
        expect(elementHasVisibleStroke(path)).toBe(false);
    });
    it("true when stroke and width set", () => {
        const el = parseSvgString(`<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0h10" stroke="black" stroke-width="2" fill="none"/></svg>`);
        const path = el.getElementsByTagName("path")[0];
        expect(elementHasVisibleStroke(path)).toBe(true);
    });
    it("false for stroke none", () => {
        const el = parseSvgString(`<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0h10" stroke="none" stroke-width="2" fill="red"/></svg>`);
        const path = el.getElementsByTagName("path")[0];
        expect(elementHasVisibleStroke(path)).toBe(false);
    });
    it("false for stroke-width 0", () => {
        const el = parseSvgString(`<svg xmlns="http://www.w3.org/2000/svg"><path d="M0 0h10" stroke="black" stroke-width="0" fill="none"/></svg>`);
        const path = el.getElementsByTagName("path")[0];
        expect(elementHasVisibleStroke(path)).toBe(false);
    });
});
