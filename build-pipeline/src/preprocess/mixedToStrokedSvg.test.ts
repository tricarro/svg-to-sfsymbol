...
describe("mixedIconToStrokedSvg", () => {
  it("emits stroked boundary paths with representative width for fill-only shapes", () => {
    ...
    expect(out).toMatch(/fill="#00f"[^>]*stroke="#00f"[^>]*stroke-width="6"/);
  });
  ...
  it("stroke+fill emits fill-only, boundary path (rep stroke), then stroke-only in order", () => {
    ...
    const iBoundaryPath = out.indexOf('stroke-linecap="round"', iFillLayer);
    const iStrokeLayer = out.indexOf('stroke="black"', iFillLayer);
    ...
    const iBoundaryTag = out.indexOf('<path fill="#f00"', iFillLayer);
    expect(iBoundaryTag).toBeGreaterThan(-1);
    expect(out.slice(iBoundaryTag, iBoundaryTag + 300)).toContain('stroke="#f00"');
    expect(out.slice(iBoundaryTag, iBoundaryTag + 300)).toContain('stroke-width="8"');
