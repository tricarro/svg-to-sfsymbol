...
const VARIABLE_SLOT_IDS = ["Ultralight-S", "Regular-S", "Black-S"] as const;
...
const MORPH_FILTER_REGION = { x: "-0.5", y: "-0.5", width: "2", height: "2" } as const;
...
function slotVisualScale(slotId: string): number {
  if (slotId === "Ultralight-S") return VARIABLE_ULTRALIGHT_SIDE / VARIABLE_ICON_BASE_SIDE;
  if (slotId === "Black-S") return VARIABLE_BLACK_SIDE / VARIABLE_ICON_BASE_SIDE;
  return 1;
}

/** Uniform scale about icon center so the center still aligns with the slot wireframe. */
function slotContentTransform(
  tx: number,
  ty: number,
  iconCenterX: number,
  iconCenterY: number,
...
export function resolveVariableTemplatePath(): string {
  ...
  if (!def) {
    throw new Error(
      `No SF Symbol variable template found. Set ${ENV_VARIABLE_TEMPLATE} to the template file path, ` +
...
function findSymbolsGroup(root: SvgElement): SvgElement {
  ...
  const g = walk(root);
  if (!g) throw new Error('No <g id="Symbols"> found in variable template.');
  return g;
}
...
function appendMorphologyFilter(
  defs: SvgElement,
  doc: SvgDocument,
  id: string,
...
function findSlot(symbols: SvgElement, slotId: string): SvgElement | null {
  for (const ch of elementChildren(symbols)) {
    if (localTag(ch) !== "g") continue;
    if (ch.getAttribute("id") === slotId) return ch;
  }
  return null;
}

function fillSlotWithVisuals(
  slot: SvgElement,
  visual: SvgElement[],
  templateDoc: SvgDocument,
  iconCenterX: number,
