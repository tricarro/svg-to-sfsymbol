...
import { elementChildren, localTag, type SvgElement } from "./xml.js";
...
const _TRANSFORM_CALL_RE =
  /(matrix|translate|scale|rotate)\s*\(([^)]*)\)/gi;

export function matIdentity(): Affine {
  return [1, 0, 0, 0, 1, 0];
}

export function matMul(m1: Affine, m2: Affine): Affine {
...
export function matTranslate(tx: number, ty: number): Affine {
  return [1, 0, tx, 0, 1, ty];
}

export function matScale(sx: number, sy: number): Affine {
  return [sx, 0, 0, 0, sy, 0];
}

export function matRotateDeg(angleDeg: number): Affine {
...
export function applyAffinePt(x: number, y: number, m: Affine): [number, number] {
  const [a, c, e, b, d, f] = m;
  return [a * x + c * y + e, b * x + d * y + f];
}

export function applyAffineZ(zre: number, zim: number, m: Affine): [number, number] {
  return applyAffinePt(zre, zim, m);
}

export function buildSizeVariantBakeMatrix(
  s: number,
  ox: number,
  oy: number,
  tx: number,
...
function floats(s: string): number[] {
  _NUM_RE.lastIndex = 0;
  const out: number[] = [];
...
function parseMatrix(args: string): Affine {
  const nums = floats(args);
  if (nums.length !== 6) throw new Error(`matrix() expects 6 numbers, got ${args}`);
  const [a, b, c, d, e, f] = nums;
  return [a, c, e, b, d, f];
}

function parseTranslate(args: string): Affine {
  const nums = floats(args);
  if (nums.length === 1) return matTranslate(nums[0], 0);
  if (nums.length === 2) return matTranslate(nums[0], nums[1]);
  throw new Error(`translate() expects 1 or 2 numbers, got ${args}`);
}

function parseScale(args: string): Affine {
  const nums = floats(args);
  if (nums.length === 1) return matScale(nums[0], nums[0]);
  if (nums.length === 2) return matScale(nums[0], nums[1]);
  throw new Error(`scale() expects 1 or 2 numbers, got ${args}`);
}

function parseRotate(args: string): Affine {
  const nums = floats(args);
  if (nums.length === 1) return matRotateDeg(nums[0]);
  if (nums.length === 3) {
    const cx = nums[1];
    const cy = nums[2];
    return matMul(
      matMul(matTranslate(cx, cy), matRotateDeg(nums[0])),
      matTranslate(-cx, -cy)
    );
  }
...
export function parseTransformAttr(raw: string | null | undefined): Affine {
  if (!raw || !raw.trim()) return matIdentity();
  let m = matIdentity();
  const calls = [...raw.matchAll(_TRANSFORM_CALL_RE)];
  if (calls.length === 0) return matIdentity();
  for (let i = calls.length - 1; i >= 0; i--) {
    const match = calls[i];
    const name = match[1].toLowerCase();
    const args = match[2];
    let part: Affine;
    if (name === "matrix") part = parseMatrix(args);
    else if (name === "translate") part = parseTranslate(args);
    else if (name === "scale") part = parseScale(args);
    else if (name === "rotate") part = parseRotate(args);
    else continue;
    m = matMul(part, m);
  }
  return m;
...
function affineToSvgMatrix(m: Affine): [number, number, number, number, number, number] {
  const [a, c, e, b, d, f] = m;
  return [a, b, c, d, e, f];
}

export function bakePathD(d: string, m: Affine): string {
...
function parsePointsAttr(raw: string | null | undefined): [number, number][] {
  if (!raw) return [];
...
export function bakeBasicShape(el: SvgElement, m: Affine): void {
  const tag = localTag(el);
  if (tag === "path") {
    const d = el.getAttribute("d");
    if (d) el.setAttribute("d", bakePathD(d, m));
    return;
  }
  if (tag === "line") {
  ...
  if (tag === "polyline" || tag === "polygon") {
    ...
    if (pts.length) {
      const baked = pts.map(([x, y]) => applyAffinePt(x, y, m)) as [number, number][];
...
export function bakeVisualSubtree(el: SvgElement, mAccum: Affine): void {
  const tag = localTag(el);
  if (tag === "g") {
