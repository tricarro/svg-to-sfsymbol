...
import { PHASE2_WEIGHTS } from "./phase2.js";
import { elementChildren, elementToBytes, localTag, parseSvgFile, svgDocumentElement, } from "./xml.js";
export const STROKE_WIDTH_BY_WEIGHT_SUFFIX = {
    "Ultralight-L": 2.5,
    "Thin-L": 4.25,
    "Light-L": 7.0,
    "Regular-L": 9.125,
...
const _WEIGHT_STEMS = new Set(PHASE2_WEIGHTS);
export function parseWeightStem(stem) {
    const i = stem.lastIndexOf("-");
    if (i < 0)
        return null;
    const weight = stem.slice(0, i);
    const suffix = stem.slice(i + 1);
    if (suffix !== "L" && suffix !== "M" && suffix !== "S")
        return null;
    if (!_WEIGHT_STEMS.has(weight))
        return null;
...
function strokeColorPresent(props) {
    const s = (props.stroke || "").trim().toLowerCase();
    return Boolean(s) && s !== "none" && s !== "transparent";
}
export function elementIsStroked(el) {
    if (!_STROKEABLE_TAGS.has(localTag(el)))
        return false;
    const strokeAttr = el.getAttribute("stroke");
    if (strokeAttr !== null) {
        const sa = strokeAttr.trim().toLowerCase();
        if (sa && sa !== "none" && sa !== "transparent")
            return true;
    }
    const style = el.getAttribute("style");
    if (style) {
        const sd = parseStyle(style);
        if (strokeColorPresent(sd))
            return true;
...
export function applyStrokeWidthsToTree(root, width) {
    let n = 0;
    const walk = (el) => {
...
function processWeightSvg(path, strokeWidth) {
...
export function runPhase3(outputDir) {
