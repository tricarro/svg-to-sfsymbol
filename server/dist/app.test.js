...
describe("convertSync", () => {
    ...
    afterAll(() => {
        if (prev === undefined)
            delete process.env.SFSYMBOL_TEMPLATE_PATH;
        else
            process.env.SFSYMBOL_TEMPLATE_PATH = prev;
    });
    ...
    it("returns merged SVG when template exists", () => {
        ...
        const svg = Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M4 12h16" stroke="black" stroke-width="2" fill="none"/></svg>`, "utf-8");
