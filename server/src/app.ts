...
export function resolveSquareTemplate(): string {
  ...
  if (!def) {
    throw new Error(
      `No SF Symbol square template found. Set ${ENV_TEMPLATE} to the template file path, ` +
...
export async function buildApp() {
  ...
  app.post("/api/convert", async (request, reply) => {
    ...
    try {
      ...
      if (
        msg.includes("SVG root must have a valid viewBox") ||
