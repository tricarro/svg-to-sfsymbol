/**
 * Mixed (fill + stroke) icons → single filled path for the variable-template pipeline.
 * Limitations: no stroke-dasharray conversion, transforms not baked, complex even-odd holes approximate.
 */
 ...
 */
export function mixedIconToFillOnlySvg(xml: string, opts?: { flatness?: number }): string {
  ...
  for (const el of collectGraphicalElements(root)) {
    ...
    if (gStroke) {
      const gg = gStroke as unknown as { isEmpty(): boolean };
      if (!gg.isEmpty()) pieces.push(gStroke);
    }
    const gFill = fillElementToGeometry(el, flatness);
    if (gFill) {
      const gg = gFill as unknown as { isEmpty(): boolean };
      if (!gg.isEmpty()) pieces.push(gFill);
    }
  }
  ...
  if (!unioned) {
    throw new Error(
      "Mixed icon conversion produced no geometry (unsupported shapes, dashed strokes, or empty paths)."
    );
  }
  const uu = unioned as unknown as { isEmpty(): boolean };
  if (uu.isEmpty()) {
